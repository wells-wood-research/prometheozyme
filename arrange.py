...
def arrange_guests(roles, unique_guests_constraints, host_path, outdir, logger=None):
    ...
    def _recursively_arrange(
        ...
        candidates_for_this_role = guests_by_role_title.get(current_role.name, [])
        ...
        for guest_uc in candidates_for_this_role:
            ...
            if any(g_info['obj'].id == guest_uc.id for g_info in current_arrangement_guests_info):
                logger.debug(f"Skipping {guest_uc.name} (ID: {guest_uc.id}) for role {current_role.name}: already in current arrangement.")
                continue

            # Skip if this guest_uc has no valid conformations 
            if not guest_uc.conformations or not os.path.exists(guest_uc.conformations):
                logger.debug(f"Skipping {guest_uc.name} (ID: {guest_uc.id}) for role {current_role.name}: no valid conformations file found at {guest_uc.conformations}.")
                continue
            ...
            for conf_idx, (total_atoms, conf_comment, conf_coords, conf_atom_types) in enumerate(all_conformations_data):
                conf_energy = parse_energy_from_comment(conf_comment, logger)
                ...
                if guest_coords_only.size == 0:
                    logger.debug(f"Skipping {guest_uc.name} (conf {conf_idx}) for role {current_role.name}: Guest coordinates are empty after slicing. Check if {guest_uc.conformations} contains host+guest.")
                    continue
